...
export async function POST(req: NextRequest) {
  try {
    ...
    const topSet = body.sets.reduce<{ w: number; r: number } | null>(
      (best, s) => {
        if (!s.completed) return best;
        const w = s.weight;
        if (!best || w > best.w) return { w, r: s.reps };
        return best;
      },
      null,
    );
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        topSet: topSet ? { weight: topSet.w, reps: topSet.r } : null,
